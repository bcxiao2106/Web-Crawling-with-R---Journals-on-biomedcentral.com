...
page.list.url = paste(site.url,substr(main.page[grep("(<a.+href).+(>Articles<\\/a>)",main.page)],79,86),"/",sep="")
...
page_index = str_extract_all(page.list[grep('<span.+data-test=\\"pagination-text\\">Page \\d+ of \\d+<\\/span>',page.list)][1],"\\d+")[[1]];
min.page.count = page_index[1];
max.page.count = page_index[2];
page.url = paste(site.url, str_extract(page.list[grep('<a class=.+c-search-navbar__pagination-link',page.list)][1],"articles\\?.+page\\="),sep="")
